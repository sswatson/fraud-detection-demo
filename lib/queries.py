...
CREATE OR REPLACE TABLE calls AS
    SELECT user_id AS sender, call_partner AS recipient, time AS timestamp FROM voc WHERE call_type = 1
    UNION ALL
    SELECT call_partner AS sender, user_id AS recipient, time AS timestamp FROM voc WHERE call_type = 2;
